...
# --- BASE DE DONNÉES DES TEXTES (1 à 9) ---
# Note : J'ai intégré ici tous les textes que vous m'avez fournis.
DATA_TEXTES = {
    1: {
        "v1": {
    ...
    4: {
        ...
        "v2": {
            ...
            "bienetre": """Attention à ne point tomber dans une certaine rigidité, tant physique que mentale, qui pourrait finir par vous fragiliser. Apprenez à assouplir votre discipline quotidienne pour ne point briser votre propre résistance interne par un excès d'exigence envers vous-même."""
        },
        "v3": {
...
# --- FONCTIONS DE SÉCURITÉ (CONSERVÉES) ---
def verifier_identifiants(email, code):
    chemin_fichier = '/home/Barfle/mysite/utilisateurs.txt'
        ...
        with open(chemin_fichier, 'r', encoding='utf-8') as f:
            for line in f:
                if ":" in line:
                    ...
                    if u.strip() == email.strip() and p.strip() == code.strip(): return True
    except: return False
    return False
...
def index():
    if request.method == 'POST':
        ...
        if verifier_identifiants(email, code):
            ...
            return render_template('index.html') # Affiche le formulaire de saisie
        return "Identifiants incorrects."
    return render_template('login.html') # Page de connexion

@app.route('/generer-pro', methods=['POST'])
def generer_pro():
    ...
    date_n = request.form.get('date')

    if not date_n: return redirect(url_for('index'))

    try:
        # 1. Calcul du départ (1er du mois suivant)
        ...
        y_naiss, m_naiss, d_naiss = map(int, date_n.split('-'))
        ...
        for i in range(12):
            ...
            a_cible = annee_depart + (mois_depart - 1 + i) // 12

            # Calcul Numérologique
            ap = reduire(d_naiss + m_naiss + a_cible)
            vib_mois = reduire(ap + (m_idx + 1))

            # Gestion des variantes v1, v2, v3
            nb = compteur_vibrations.get(vib_mois, 0) + 1
            compteur_vibrations[vib_mois] = nb
            version = f"v{min(nb, 3)}"

            t = DATA_TEXTES[vib_mois][version]

            rapport_html += f"""
            <div style="margin-bottom: 40px; padding: 20px; border-left: 5px solid #d4af37; background: #f9f9f9;">
                <h3 style="color: #d4af37;">✨ {mois_noms[m_idx]} {a_cible} (Vibration {vib_mois})</h3>
                <p><strong>💼 Pro :</strong> {t['pro']}</p>
                <p><strong>❤️ Cœur :</strong> {t['coeur']}</p>
                <p><strong>💰 Flux :</strong> {t['argent']}</p>
                <p><strong>🌿 Énergie :</strong> {t['bienetre']}</p>
            </div>
            """

        # Affichage final (On utilise un template dédié pour le résultat)
