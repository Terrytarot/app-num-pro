...
if submit:
    if nom and prenom:
        ...
        st.header(f"✨ Bonjour {prenom}, voici votre année {annee_en_cours}")
        ...
        for m in range(1, 13):
            ...
            with st.expander(f"📅 {obtenir_nom_mois(m).upper()} - VIBRATION {chiffre_mois}", expanded=(m==1)):
                st.markdown(f"**💼 Vie Professionnelle**")
                st.write(textes["pro"])
                st.markdown(f"**❤️ Vie Affective**")
                st.write(textes["coeur"])
                st.markdown(f"**💰 Argent & Abondance**")
                st.write(textes["argent"])
                st.markdown(f"**🌿 Bien-être & Énergie**")
